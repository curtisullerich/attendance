...
<!--
   function newMessage() {
		...
		$.post($("#newmessageform").attr('action'), {'id':threadid, 'Message':text});
		...
		//switch those radio buttons
		$('#resolved_radio').attr('checked', 'unchecked');
		$('#unresolved_radio').attr('checked', 'checked');
		$('#studenttext').html("<i>Not Resolved.</i>");
	...
	function changeResolved()
	{
		...
		$.post($("#threadresolveform").attr('action'), {'id':threadid, 'resolved':resolved});
		
		if(resolved == "false") {
			$("#newmessagepreview").html("Click here to send a reply and <strong>un-resolve</strong> the thread");
			$('#studenttext').html("<i>Resolved.</i>");
		}
		else {
 ...
 // -->
			...
			<c:choose>
				<c:when test="${auth.user.type.director}">
					<div class="threadresolved">
						<form id="threadresolveform" action='/director/messages/handle' method="post" accept-charset="utf-8" onsubmit="return changeResolved();">
				...
				<c:when test="${thread.resolved}">
					<p id="studenttext" style="font-size:large;"><i>Resolved.</i></p>
				</c:when>
				<c:when test="${!thread.resolved}">
					<p id="studenttext" style="font-size:large;"><i>Not Resolved.</i></p>
				</c:when>
			</c:choose>
		<br/>
		...
		<div class="grid-full">
			...
			<form id="newmessageform" method="post" action='/director/messages/handle' accept-charset="utf-8" style="display:${(empty thread.messages)?'block':'none'};"
				onsubmit="return newMessage();">
			</c:if>
			<c:if test="${!auth.user.type.director}">
			<form id="newmessageform" method="post" action='/student/messages/handle' accept-charset="utf-8" style="display:${(empty thread.messages)?'block':'none'};"
				onsubmit="return newMessage();">
			</c:if>
